...
export function AppChrome({
  ...
  return (
    <header
      className="sm-chrome z-10 flex shrink-0 items-center justify-center gap-2 border-b border-black/5 bg-transparent px-3 py-2.5"
      style={{
    ...
    >
      <div className="sm-glass flex w-full max-w-[95%] items-center gap-1 rounded-2xl px-1.5 py-1 sm:max-w-[85%] sm:gap-2 sm:px-2 sm:py-1.5 text-sm">
        <form
          method="GET"
          action="/search"
        ...
        {email ? (
          <>
            <Link href="/" className={navClass(active === "board")}>
              Board
            </Link>
            <Link
              href="/messages"
              className={navClass(active === "messages")}
            >
              Messages
            </Link>
            <Link
